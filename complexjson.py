...
class BidirectionalDict(object, DictMixin):
    ...
    def _pop():
        def pop(original, referenced, key, *a):
            try:
                ...
                return target
        def lpop(self, lv, *a):
            return pop(self.__l2r, self.__r2l, lv, *a)
        def rpop(self, rv, *a):
            return pop(self.__r2l, self.__l2r, rv, *a)
        return lpop, rpop
    ...
    def _parse_slice(self, k):
        ...
        if k.step is None:
            ...
            if (lv is not None and rv is None) or \
                ...
                return lv, rv
        raise TypeError('BidirectionalDict only accept slices like d[lv:] -> rv'
                        ' or d[:rv] -> rv, got [{0}:{1}:{2}]'.format(
...
class JSONConverter(object):
    ...
    def register(self, key_or_cls, cls=None):
        ...
        if not hasattr(cls, self.json_key):
            raise TypeError("type must have {0} method defined".format(
                self.json_key))
        self.__types[key] = cls
        return cls

    def freeze(self):
        self.types = tuple(self.__types.itervalues())

    def encode_type(self, o):
        type = type(o)
        if type not in self.types:
            return self.default(o)
        return {self.type_key: self.__types.rget(type),
                self.data_key: getattr(type, self.json_key)(o)}
    ...
    def decode_type(self, d):
        ...
        type = self.__types[type_idx]
        return getattr(type, self.json_key)(data)
